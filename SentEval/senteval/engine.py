...
from senteval.cn.atec import ATECEval
from senteval.cn.bq import BQEval
from senteval.cn.lcqmc import LCQMCEval
from senteval.cn.pawsx import PAWSXEval
from senteval.cn.sts_benchmark import ChineseSTSBenchmarkEval


class SE(object):
    def __init__(self, params, batcher, prepare=None, tokenizer=None):
        # parameters
        ...
        self.list_tasks = ['CR', 'MR', 'MPQA', 'SUBJ', 'SST2', 'SST5', 'TREC', 'MRPC',
                           ...
                           'CN-ATEC', 'CN-BQ', 'CN-LCQMC', 'CN-PAWSX', 'CN-STSBenchmark']
    ...
    def eval(self, name):
        ...
        # Chinese Senteval tasks
        elif name == 'CN-ATEC':
            self.evaluation = ATECEval(tpath + '/downstream/CHINESE/ATEC', tokenizer=self.tokenizer, seed=self.params.seed)
        elif name == 'CN-BQ':
            self.evaluation = BQEval(tpath + '/downstream/CHINESE/BQ', tokenizer=self.tokenizer, seed=self.params.seed)
        elif name == 'CN-LCQMC':
            self.evaluation = LCQMCEval(tpath + '/downstream/CHINESE/LCQMC', tokenizer=self.tokenizer, seed=self.params.seed)
        elif name == 'CN-PAWSX':
            self.evaluation = PAWSXEval(tpath + '/downstream/CHINESE/PAWSX', tokenizer=self.tokenizer, seed=self.params.seed)
        elif name == 'CN-STSBenchmark':
            self.evaluation = ChineseSTSBenchmarkEval(tpath + '/downstream/CHINESE/STS-B', tokenizer=self.tokenizer, seed=self.params.seed)
